...
from .models import BlogTracking
import uuid
import json
import requests
...
def blog(request):
    try:
        ...
        {
            'id': 4,
            'title': 'Home Staging: Maximize Your Property Value',
            'author': 'Sarah Wilson',
            'date': date(2023, 9, 20),
            'image_url': 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=400&h=300&fit=crop',
            'excerpt': 'Professional tips on how to stage your home to attract buyers and increase property value.',
        },
        {
            'id': 5,
            ...
            'date': date(2023, 9, 10),
            'image_url': 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=400&h=300&fit=crop',
            'excerpt': 'A comprehensive guide to different mortgage types and how to choose the right one for you.',
        },
        {
            'id': 6,
            'title': 'Luxury Real Estate: What Buyers Want',
            'author': 'Emily Davis',
            'date': date(2023, 8, 25),
        ...
        # Return a simple error response for debugging
        return HttpResponse(f"Error in blog view: {str(e)}", status=500)


def blog_detail(request, post_id):
    # Sample blog posts data (same as in blog view)
    posts = [
        {
            'id': 1,
            'title': 'Exploring Real Estate Investment Opportunities',
